...
if ($result->num_rows == 1) {
    ...
    echo "Data $get_kode tidak tersedia";
    exit;
}
...
if (isset($_POST['submit'])) {
    ...
    $get_kode = $_POST['kode_mk'];
    $nama_matkul = $_POST['nama_matkul'];
    $ruang = $_POST['id_ruang']; 
    $dosen = $_POST['id_dosen'];

    // Persiapkan dan jalankan SQL untuk memperbarui data
    $query = "UPDATE matkul SET  nama_matkul=?, id_dosen=?, id_ruang=? WHERE kode_mk=?";
    $stmt = $db->prepare($query);
    $stmt->bind_param("sssi",  $nama_matkul, $dosen, $ruang,   $get_kode);
      ...
      <div class="card-body">
        <h5 class="card-title">Mata Kuliah</h5>
            ...
            <form class="row g-3" method="post">
                          <div class="col-md-6">
                            <label for="inputName5" class="form-label" >Kode Matkul</label>
                          ...
                          <div class="col-md-6">
                            ...
                            <?php
                                ...
                                while ($data_dosen = mysqli_fetch_array($dosen)) {
                                  $select =$data_dosen['id'] == $row['id_dosen'] ? 'selected' : '';
                                    echo "<option $select value=" . $data_dosen['id'] . ">" . $data_dosen['nama'] . "</option>";
                                }
                            ?>
                            </select>
                          </div>
                          ...
                          <div class="col-md-6">
                            ...
                            <?php
                                ...
                                while ($data_ruang = mysqli_fetch_array($ruang)) {
                                  $select =$data_ruang['id'] == $row['id_ruang'] ? 'selected' : '';
                                    echo "<option $select value=" . $data_ruang['id'] . ">" . $data_ruang['nama_ruang'] . "</option>";
                                }
                            ?>
                        </select>
                          </div>
